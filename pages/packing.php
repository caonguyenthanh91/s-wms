...
<style>
    ...
    .pda-btn-primary:hover:not(:disabled) {
        background: #0369a1;
    }
    ...
    .pda-btn-success:hover:not(:disabled) {
        background: #15803d;
    }
...
<div class="pda-pack-wrap pb-4">
    ...
    <div id="step-1" class="pda-step active pda-card p-3 mb-3">
        ...
        <div class="pda-title mt-1">Quét QR invoice</div>
        <div class="text-xs text-slate-500 mt-1">Định dạng bắt buộc: [command 6 ký tự][case_no 3 ký tự]. Ví dụ: ABCDEF001</div>

        <div class="relative mt-3">
            <input type="text" id="invoice-input" class="pda-input" placeholder="VD: ABCDEF001" maxlength="9">
...
function updateStatusCards() {
    const required = packingState.requiredTotal || 0;
    const st = packingState.statusTotals || { picking: 0, packing: 0, pickup: 0 };

    $('#state-picking-value').text(`${st.picking || 0} / ${required}`);
    $('#state-packing-value').text(`${st.packing || 0} / ${required}`);
    $('#state-pickup-value').text(`${st.pickup || 0} / ${required}`);

    $('#state-picking').toggleClass('done', required > 0 && (st.picking || 0) >= required);
    $('#state-packing').toggleClass('done', required > 0 && (st.packing || 0) >= required);
    $('#state-pickup').toggleClass('done', required > 0 && (st.pickup || 0) >= required);
}
...
function handleInvoiceScan() {
    ...
    if (!parsed) {
        showError('#step1-error', 'Invoice không hợp lệ. Cần dùng 9 ký tự: [command 6][case_no 3].');
        return;
    }
